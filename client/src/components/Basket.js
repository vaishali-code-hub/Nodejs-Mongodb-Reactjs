...
function Basket(props) {
  ...
  let itemPrice = cart.reduce((sum, currentItem) => {
    return currentItem["Variant Price"] === ""
      ? sum + productsPrice_WithSameHandle["Variant Price"] * currentItem.qty
      : sum + currentItem["Variant Price"] * currentItem.qty;
  }, 0);
  ...
  return (
    <aside className="block col-1">
      <h2>Cart Items</h2>
      <div>{cart.length === 0 && <div>Empty</div>}</div>
      {cart.map((item) => (
        <div className="row">
